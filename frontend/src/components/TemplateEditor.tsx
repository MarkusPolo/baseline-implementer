...
interface TemplateEditorProps {
    initialSteps: string[] | TemplateStep[];
    initialName?: string;
    onSave: (name: string, steps: TemplateStep[], schema: TemplateSchema) => void;
    onCancel: () => void;
}

export function TemplateEditor({ initialSteps, initialName = "", onSave, onCancel }: TemplateEditorProps) {
    const [name, setName] = useState(initialName);

    const normalizeSteps = (steps: string[] | TemplateStep[]): TemplateStep[] => {
        if (steps.length === 0) return [];
        if (typeof steps[0] === 'string') {
            return (steps as string[]).map(cmd => ({ type: "send", cmd, wait_prompt: true }));
        }
        return (steps as TemplateStep[]).map(step => {
    ...
    const [steps, setSteps] = useState<TemplateStep[]>(normalizeSteps(initialSteps));
    ...
    const generateSchema = (currentSteps: TemplateStep[]) => {
        ...
        const schema: TemplateSchema = { type: "object", properties: {}, required: [] };
        vars.forEach(v => {
    ...
    const updateStep = (index: number, field: keyof TemplateStep, value: TemplateStep[keyof TemplateStep]) => {
